...
run_model <- function(target_column, new_input) {
  ...
  svr_model <- svm(
    x = train_features,
    y = train_target,
    kernel = "radial",  # You can choose a different kernel (e.g., "linear", "polynomial")
    cost = 1,           # Cost parameter
    epsilon = 0.1       # Epsilon parameter
  )
  
  # Predict
  preds <- predict(svr_model, newdata = test_features)
  ...
  prediction <- predict(svr_model, newdata = new_input)
